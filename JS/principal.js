...
// Seleciona todos os cartões de jogo
const cartoes = document.querySelectorAll('.cartao-jogo');

cartoes.forEach((cartao, index) => {
  const jogo = jogos[index];
  const botaoPlay = cartao.querySelector('.botao-descricao .fa-play');
  const imagem = cartao.querySelector('img');

  if (jogo && jogo.link) {
    // Linka o botão de play
    botaoPlay.parentElement.addEventListener('click', () => {
      window.location.href = jogo.link;
    });

    // Linka a imagem também
    imagem.addEventListener('click', () => {
      window.location.href = jogo.link;
    });
  } else {
    // Se não tiver link, desativa o botão
    botaoPlay.parentElement.style.opacity = '0.5';
    botaoPlay.parentElement.style.cursor = 'not-allowed';
    botaoPlay.parentElement.title = 'Em breve!';
  }
});
